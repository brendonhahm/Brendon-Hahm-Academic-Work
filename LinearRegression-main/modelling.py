...
class Model:
    def __init__(self, df, VOI, controls, resp, alpha = .05, interactions = None, dummies = None, learning_rate = .01, n_iterations = 1000):
        self.df = df
        self.VOI = VOI
                    ...
                    if col == dum:
                        ...
                        for val in vals:
                            ...
                            if first == True:
                                first = False
                                continue
                            self.df[val] = [1 if x == val else 0 for x in self.df[col]]
    ...
    def __str__(self):
        return f'OLS model measuring effect of {self.VOI} on {self.resp}'
